#!/usr/bin/env node
// send_triggered.js — send outreach only if fresh evidence exists; dynamic subject/body by type
const fs = require('fs'); const path = require('path'); const nodemailer = require('nodemailer');
...
function leads(){ const arr = (lines(path.join('data','leads.csv')).map(parseLead).filter(Boolean) || []).filter(x=>!['unsub','optout','bounced','invalid','bad-mx'].includes(x.status)); return arr; }
function detectType(obj,fname=''){ const t=(JSON.stringify(obj||{}).toLowerCase()+' '+fname.toLowerCase());
  if(/pricing|price|plan/.test(t)) return 'Pricing'; if(/\btos\b|terms of service|terms/.test(t)) return 'ToS';
  if(/\bdpa\b|data processing/.test(t)) return 'DPA'; if(/subprocessor|sub-?processor/.test(t)) return 'Subprocessors';
  if(/status|incident|uptime/.test(t)) return 'Status'; return 'Public change';
}
function freshEvidence(windowH){ const base='evidence', min=Date.now()-windowH*3600*1000, out=[]; if(!fs.existsSync(base)) return out;
...
function subjectOf(changes){ if(changes.length===1) return `[${changes[0].type}] ${changes[0].slug} updated — evidence inside`;
  const map={}; for(const c of changes){ map[c.type]=map[c.type]||new Set(); map[c.type].add(c.slug); }
  const parts=Object.entries(map).map(([t,sl])=>`${t}(${[...sl].slice(0,3).join('+')}${sl.size>3?'+…':''})`); return `Recent changes: ${parts.slice(0,3).join(' · ')}`;
}
function bodyHTML(changes, windowH){ const lis = changes.slice(0,6).map(c=>`<li><b>[${c.type}]</b> ${c.slug} — <a href="${vendorsURL(c.slug)}">evidence</a> <span style="color:#666">(${c.when.toISOString().slice(0,10)})</span></li>`).join('\n');
  return `<!doctype html><html><body style="font-family:system-ui,Segoe UI,Roboto,Arial"><p>Hi, we detected verifiable public changes in the past ${windowH} hours:</p><ul>${lis||'<li>No recent changes.</li>'}</ul><p style="margin-top:16px">We monitor Pricing / ToS / DPA / Subprocessors / Status.</p><p><a href="https://www.cg-alert.com/updates/?utm=outreach_triggered">View more updates</a></p><hr style="border:none;border-top:1px solid #eee"><p style="color:#666;font-size:12px">Unsubscribe: reply "STOP" or click <a href="mailto:${MAIL_FROM}?subject=Unsubscribe">here</a>.</p></body></html>`;
}
(async function main(){
  const windowH = Number(argv['window-h']); const limit = Number(argv.limit); const dry = !!argv.dry;
  const changes = freshEvidence(windowH); if(!changes.length){ console.log(`[triggered] no fresh evidence in ${windowH}h → exit`); return; }
  const list = leads().filter(x=>x.mx_ok!==false).slice(0, limit); if(!list.length){ console.log('[triggered] no eligible leads'); return; }
  ...
  const subject = subjectOf(changes); const html = bodyHTML(changes, windowH);
  const headers = { 'List-Unsubscribe': `<mailto:${MAIL_FROM}?subject=unsubscribe>`, 'List-Unsubscribe-Post': 'List-Unsubscribe=One-Click' };
  let sent=0; for(const lead of list){ if(dry){ console.log(`[dry] ${lead.email} ← ${subject}`); continue; }
