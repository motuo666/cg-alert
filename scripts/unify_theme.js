...
const header = fs.readFileSync(path.join('fragments','header.html'),'utf8');
const footer = fs.readFileSync(path.join('fragments','footer.html'),'utf8');

const targets = ['.', 'reports', 'who-uses', 'terms', 'privacy', 'seo'];
function* walk(dir){
...
function injectHF(text){
  ...
  if(!/footer class="site"/.test(out)) out = out.replace(/<\/body>/i, footer + '\n</body>');
  return out;
}
...
console.log('theme unify complete');
