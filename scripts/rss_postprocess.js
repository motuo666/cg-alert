...
function ensurePi(xml){
  xml = xml.replace(/<\?xml-stylesheet[^>]*\?>\s*/ig, '');
...
function main(){
  ...
  if(doc.rss){
    ...
    doc.rss['@_xmlns:cg'] = doc.rss['@_xmlns:cg'] || 'https://www.cg-alert.com/ns';
    const ch = doc.rss.channel || (doc.rss.channel = {});
    let items = ch.item || [];
