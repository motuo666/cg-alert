...
function readEvidence(){
  ...
  for(const slug of fs.readdirSync(EVD)){
    const dir=path.join(EVD,slug); if(!fs.statSync(dir).isDirectory()) continue;
    for(const f of fs.readdirSync(dir)){
      if(!/^\d{4}-\d{2}-\d{2}-/.test(f)||!f.endsWith('.json')) continue;
      const dateStr=f.slice(0,10),type=f.replace('.json','').split('-')[3]||'';
      const ym=`${dateStr.slice(0,4)}-${dateStr.slice(5,7)}`;
      items.push({slug,date:dateStr,ym,type,file:path.join(dir,f)});
    }
  }
  items.sort((a,b)=>a.date<b.date?1:-1); return items;
}
function buckets(items){
  const m=new Map(); for(const it of items){ if(!m.has(it.ym)) m.set(it.ym,[]); m.get(it.ym).push(it); }
...
h1{font-size:28px;margin:0 0 12px}.muted{color:var(--muted);font-size:14px}
...
.right{text-align:right}footer{margin-top:36px;font-size:13px;color:var(--muted)}
</style>
...
function writeIndex(months){
  ...
  const canonical=`${ORIGIN}/reports/`, latest=months[0]?.[0];
  const list=months.slice(0,MAX_MONTHS_ON_INDEX).map(([ym])=>`<li><a href="${ym}/">${ym}</a></li>`).join('');
  const html=[
    head({title:'Reports',canonical,breadcrumbs:[{name:'Reports',url:canonical}]}),
    `<h1>Reports</h1>`,
    latest?`<div class="muted">Latest: <a href="${latest}/">${latest}</a></div>`:`<div class="muted">No reports yet.</div>`,
    `<ul class="list">${list}</ul>`, foot()
  ].join('');
...
function writeMonth(ym,items){
  ...
  const lis=items.map(it=>`<li>${it.date} · <a href="${ORIGIN}/vendors/${it.slug}/">${esc(it.slug)}</a></li>`).join('');
  const topTable=top.length?`<table class="grid"><thead><tr><th>Vendor</th><th class="right">Changes</th></tr></thead><tbody>${
    top.map(([slug,c])=>`<tr><td><a href="${ORIGIN}/vendors/${slug}/">${esc(slug)}</a></td><td class="right">${c}</td></tr>`).join('')
  }</tbody></table>`:'';
  const html=[
    head({title:`Public Changes — ${ym}`,canonical,breadcrumbs:[{name:'Reports',url:`${ORIGIN}/reports/`},{name:ym,url:canonical}]}),
    ...
    topTable, `<ul class="list">${lis}</ul>`, foot()
  ].join('');
...
function writeRSS(months){
  const items=months.slice(0,MAX_MONTHS_ON_INDEX).map(([ym,arr])=>{
    const link=`${ORIGIN}/reports/${ym}/`,title=`Public Changes — ${ym}`;
    const pub=new Date(arr[0]?.date||`${ym}-01`).toUTCString();
    return `<item><title>${esc(title)}</title><link>${link}</link><guid>${link}</guid><pubDate>${pub}</pubDate></item>`;
  }).join('');
  const rss=`<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel>
<title>CG Alert · Reports</title><link>${ORIGIN}/reports/</link><description>Monthly public changes across monitored vendors</description>${items}
</channel></rss>`;
...
function writeSitemap(months){
  ...
  const xml=`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>${ORIGIN}/reports/</loc></url>${urls}</urlset>`;
  fs.writeFileSync(path.join(ROOT,'sitemap-reports.xml'),xml,'utf8');
  const files=['sitemap-pages.xml','sitemap-vendors.xml','sitemap-updates.xml','sitemap-reports.xml'].filter(f=>fs.existsSync(path.join(ROOT,f)));
...
(function main(){
  const all=readEvidence(), months=buckets(all); ensure(OUT);
  if(months.length===0){
    fs.writeFileSync(path.join(OUT,'index.html'),'<!doctype html><meta charset="utf-8"><title>Reports</title><div class="wrap"><div class="nav"><a href="'+ORIGIN+'/">Home</a> · <a href="'+ORIGIN+'/reports/">Reports</a></div><h1>Reports</h1><p>No reports yet.</p></div>');
    writeRSS([]); writeSitemap([]); console.log('reports: empty'); return;
  }
  writeIndex(months); for(const [ym,arr] of months) writeMonth(ym,arr);
  writeRSS(months); writeSitemap(months); console.log(`reports: index + ${months.length} months built`);
})();
