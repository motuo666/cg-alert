...
function updateHrefByPlanGuess(html, planKeyword, href) {
  const re = new RegExp(`(<a[^>]*href=["'][^"']*["'][^>]*>[^<]{0,120}</a>)`, 'ig');
  let m; let lastIndex = 0; let out = ''; let changed = false;
  while ((m = re.exec(html)) !== null) {
    ...
    if (new RegExp(planKeyword, 'i').test(context)) {
      ...
      const newAnchor = anchorRe.test(anchor) ? anchor.replace(anchorRe, `$1${href}$2`) : anchor;
