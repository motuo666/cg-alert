...
# Match patterns like: grep -RIn ... -F 'conversation too long; truncated' .
# Use awk to rewrite the exact line for portability across macOS/Linux sed variations.
awk '
  BEGIN{re=/"conversation too long; truncated"/}
  {
    if ($0 ~ /grep[[:space:]]+-RIn/ && $0 ~ re) {
      print "          if grep -RIn --exclude-dir=.git --exclude-dir=.github --exclude-dir=scripts --exclude="*/final-acceptance.yml" --exclude="scripts/ci/scan_placeholders.sh" -F '\''conversation too long; truncated'\'' . ; then";
    } else {
      print $0;
    }
  }
