...
const N = Math.max(1, Number(process.argv[2]||80));
...
const S=new Set(had), add=[];
for(const d of pool){ if(!S.has(d)){ add.push(d); S.add(d); if(add.length>=N) break; } }
if(!add.length){ console.log('[expand] no new domains'); process.exit(0); }
fs.appendFileSync(domF, add.join('\n')+'\n','utf8');
console.log(`[expand] +${add.length} domains (now ${S.size})`);
