#!/usr/bin/env node
/**
 * CG Alert — Outreach Sender (email-only, DRY-safe)
...
const LIMIT = parseInt(process.env.SEND_LIMIT || '12', 10);
...
function hmac(s){
  return crypto.createHmac('sha256', UNSUB_HMAC_SECRET).update(String(s)).digest('hex');
}

function body(contact){
  const unsub = `${UNSUB_ORIGIN}/unsubscribe/?u=${encodeURIComponent(contact.email)}&s=${hmac(contact.email)}`;
  const html = `<p>Hey ${contact.name||''},</p>
<p>We monitor vendor <b>Pricing/ToS/DPA/Subprocessors/SLA</b> with evidence cards (URL · timestamp · SHA256).</p>
<p>Use at renewal to push back on surprise uplifts or terms.</p>
<p>— Jason @ CG Alert</p>
<p style="font-size:12px;color:#666">Unsubscribe: <a href="${unsub}">${unsub}</a></p>`;
  const text = `We monitor vendor Pricing/ToS/DPA/Subprocessors/SLA with evidence cards.
Use at renewal to push back. Unsub: ${unsub}`;
  ...
  for(const r of take){
    ...
    try{
      if(!DRY && transport){
        ...
        const mail = {
          from: FROM, to: r.email, subject: 'Evidence-backed vendor change alerts',
          html, text, headers,
          envelope: RETURN_PATH ? { from: RETURN_PATH, to: r.email } : undefined,
